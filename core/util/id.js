...
class IDUtil {
   ...
   *    An attribute of the object that should checked rather than the object keys
  ...
  incrementStringIfNeeded(string) {
    ...
    if (matches) {
      ...
      slugNum = parseInt(matches[1], 10) + 1
  ...
  /**
   * [trimObject Deletes all keys/values from the first
   *    object that don't exist in the second object]
  ...
  trimObject(obj1, obj2) {
    for (const key in obj1) {
      ...
      if (key.length > 0 && key[0] !== '_') {
        if (obj2[key] == null) {
          delete obj1[key] // eslint-disable-line
